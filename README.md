...
**可能原因**：
- 分享链接已失效或被取消
- 分享文件夹空间不足（单个文件或总大小超过当前可用空间）
- Cookie 不完整（缺少某些域名或未重新复制）
- API 调用临时失败（重试通常有效）

**解决方法**：
1. 在浏览器中直接打开链接，确认是否能正常查看文件列表和下载。
2. 重新复制一个**最新**的 Cookie（浏览器无痕模式 F12 → Network → list 请求头）。
3. 确保分享链接格式正确（无多余字符）。
4. 如果空间不足，扩展会自动跳过超大文件；如果无法解决，可降低 `最大解析文件数量` 设置。

---

**项目已更新**：代码已优化，版本升级到 **1.0.5**，推送已准备好。
